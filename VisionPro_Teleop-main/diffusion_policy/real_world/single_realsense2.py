...
class SingleRealsense(mp.Process):
    ...
    def __init__(
        ...
        if recording_transform is None:
            self.video_recorder_example = examples["color"]
        else:
            self.video_recorder_example = recording_transform(dict(examples))["color"]
    ...
    def run(self):
        ...
        if self.verbose:
            print(f"[SingleRealsense {self.serial_number}] Debug Point 1.")
        ...
        try:
            ...
            intr = color_stream.as_video_stream_profile().get_intrinsics()  # 内参
            ...
            print(f"[SingleRealsense {self.serial_number}] Exception: {e}")
        finally:
            self.video_recorder.stop()
