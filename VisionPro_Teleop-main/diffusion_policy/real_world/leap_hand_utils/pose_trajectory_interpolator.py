...
class PoseTrajectoryInterpolator:
    ...
    def trim(self, start_t: float, end_t: float) -> "PoseTrajectoryInterpolator":
        """The trim method creates a new `PoseTrajectoryInterpolator` that only includes poses between specified start and end times. It ensures the times are unique and interpolates the poses at these times.
        ...
        Returns:
            PoseTrajectoryInterpolator: new `PoseTrajectoryInterpolator`
        """
        ...
        # interpolate
        all_poses = self(all_times)  # use __call__ to interpolate
    ...
    def drive_to_waypoint(
        ...
        """The `drive_to_waypoint` method generates a new interpolator that drives the current pose to a specified waypoint within a given time, considering maximum positional and rotational speeds. It calculates the required duration to reach the waypoint and creates a new interpolator with the updated poses.

        Args:
            pose (16, ): a specified waypoint
            time (float): given time to reach the waypoint
            curr_time (float): current time
            max_pos_speed (float, optional):  Defaults to np.inf.
            max_rot_speed (float, optional):  Defaults to np.inf.

        Returns:
            PoseTrajectoryInterpolator: _description_
        """
        ...
        pos_dist = np.max(np.abs(pose - curr_pose))
    ...
    def schedule_waypoint(
        ...
        pos_dist = np.max(np.abs(pose - end_pose))
        pos_min_duration = pos_dist / max_pos_speed
        duration = max(duration, pos_min_duration)
