...
class DynamixelClient:
    ...
    def read_pos(self) -> np.ndarray:
        """Returns the current positions and velocities."""
        return self._pos_reader.read(retries=1)
    ...
    def write_desired_vel(self, motor_ids: Sequence[int], velocities: np.ndarray):
        ...
        # Convert to Dynamixel velocity space.
    ...
    def sync_write(
        self, motor_ids: Sequence[int], values: Sequence[Union[int, float]], address: int, size: int
    ):
...
class DynamixelReader:
    ...
    def __init__(self, client: DynamixelClient, motor_ids: Sequence[int], address: int, size: int):
        ...
        self.client.port_handler.setPacketTimeoutMillis(1)  # 1ms
        # client.port_handler.setPacketTimeout()
        self.operation = self.client.dxl.GroupSyncRead(
            self.client.port_handler, self.client.packet_handler, address, size
        )
        ...
        for motor_id in motor_ids:
            ...
            if not success:
                raise OSError(
                    "[Motor ID: {}] Could not add parameter to bulk read.".format(motor_id)
                )

    def read(self, retries: int = 4):
...
class DynamixelFastReader:
    ...
    def __init__(self, client: DynamixelClient, motor_ids: Sequence[int], address: int, size: int):
        ...
        self.operation = self.client.dxl.GroupFastSyncRead(
            client.port_handler, client.packet_handler, address, size
        )
        ...
        for motor_id in motor_ids:
            ...
            if not success:
                raise OSError(
                    "[Motor ID: {}] Could not add parameter to sync read.".format(motor_id)
                )
    ...
    def read(self, retries: int = 1):
        ...
        if errored_ids:
            logging.error("Fast sync read data is unavailable for: %s", str(errored_ids))
            print("Fast sync read data is unavailable for: %s", str(errored_ids))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--device", default="/dev/ttyUSB0", help="The Dynamixel device to connect to."
    )
    ...
    with DynamixelClient(motors, parsed_args.device, parsed_args.baud) as dxl_client:
        for step in itertools.count():
            ...
            if step % 5 == 0:
                print("[{}] Frequency: {:.2f} Hz".format(step, 1.0 / (time.time() - read_start)))
