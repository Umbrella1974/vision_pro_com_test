...
class RTDEInterpolationController(mp.Process):
    ...
    def __init__(
        ...
        lookahead_time=0.2,
        gain=300,
        max_pos_speed=0.2,  # 5% of max speed
        max_rot_speed=0.1,  # 5% of max speed
        ...
        if tcp_init_pose is not None:
            tcp_init_pose = np.array(tcp_init_pose)
            assert tcp_init_pose.shape == (6,)
        ...
        self.tcp_init_speed = tcp_init_speed
        ...
        self.WORKSPACE_LIMIT_WORLD_MIN_XYZ = workspace_min_xyz_W
        self.WORKSPACE_LIMIT_WORLD_MAX_XYZ = workspace_max_xyz_W
    ...
    def safety_check(self, target_pose: np.ndarray) -> np.ndarray:
        target_position = target_pose[:3]
        target_position_clip = np.clip(
            target_position,
            self.WORKSPACE_LIMIT_WORLD_MIN_XYZ,
            self.WORKSPACE_LIMIT_WORLD_MAX_XYZ,
        )
        return np.hstack([target_position_clip, target_pose[3:]])
    ...
    def run(self):
        ...
        try:
            ...
            if self.tcp_init_pose is not None:
                assert rtde_c.moveL(self.tcp_init_pose, self.tcp_init_speed)
            ...
            while keep_running:
                ...
                for i in range(n_cmd):
                    ...
                    if cmd == Command.STOP.value:
                        ...
                        pose_interp = pose_interp.drive_to_waypoint(
                            pose=self.safety_check(target_pose),
                            time=t_insert,
                            curr_time=curr_time,
                ...
                if self.verbose:
                    print(
                        f"[RTDEPositionalController] Actual frequency {1/(time.perf_counter() - t_start)}"
                    )
        except Exception as e:
            print(f"[RTDEPositionalController] Error: {e}")
